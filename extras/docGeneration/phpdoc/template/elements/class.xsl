<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/project/file/class|/project/file/interface" mode="sidebar">
        ...
        <div class="btn-group view" data-toggle="buttons-radio">
          <button class="btn simple active" title="Show only method names"><i class="icon-align-justify"></i></button>
    ...
    <xsl:template match="class|interface" mode="contents">
        ...
        <div href="{$root}classes/{substring($link, 2)}.html" class="element {local-name()}">
            <xsl:variable name="fullNamespace" select="full_name"/>
            <xsl:if test="starts-with($fullNamespace, '\RightNow\Models')">
                <div class="warning">
                    <strong>IMPORTANT NOTE:</strong> Unless specifically stated otherwise, all model methods actually return an instance of a
                    <a href="RightNow.Libraries.ResponseObject.html">RightNow\Libraries\ResponseObject</a>. The @return documentation for each method below denotes the
                    type of value that will be within the 'result' property of the ResponseObject if no errors were encountered.
                </div>
            </xsl:if>
