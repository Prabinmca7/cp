<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="argument">
        <div class="subelement argument">
            ...
            <xsl:if test="$default != ''">
                <h4><xsl:text>[</xsl:text><xsl:value-of select="name"/><xsl:text> = </xsl:text><xsl:value-of select="default"/><xsl:text>]</xsl:text></h4>
            </xsl:if>
            <xsl:if test="$default = ''">
    ...
    <xsl:template match="function|method" mode="contents">
        ...
        <xsl:variable name="inheritedMethod">
            <xsl:if test="inherited_from">inherited</xsl:if>
        </xsl:variable>
        <div class="element clickable {local-name(.)} {@visibility} {name} {$inheritedMethod}" data-toggle="collapse" data-target=".{name} .collapse">
