<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="property|constant" mode="contents">
        ...
        <xsl:variable name="inheritedConstant">
            <xsl:if test="inherited_from">inherited</xsl:if>
        </xsl:variable>
        <div class="element clickable {local-name(.)} {@visibility} {name} {$inheritedConstant}" data-toggle="collapse" data-target=".{name} .collapse">
