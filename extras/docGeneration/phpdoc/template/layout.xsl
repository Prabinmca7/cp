<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/project//namespace" mode="menu">
        ...
        <xsl:if test="contains($link, '.')">
            <xsl:if test="not(contains(substring-after($link, '.'), '.'))">
                <li>
                    <a href="{$root}namespaces/{$link}.html">
                        <span style="padding-left:18px"><i class="icon-th"></i>&#160;<xsl:value-of select="$title" /></span>
                    </a>
                </li>
            </xsl:if>
        </xsl:if>
        <xsl:if test="not(contains($link, '.'))">
            <li>
                <a href="{$root}namespaces/{$link}.html">
                    <i class="icon-th"></i>&#160;<xsl:value-of select="$title" />
                </a>
            </li>
        </xsl:if>
    </xsl:template>
            ...
            <div class="navbar-inner">
                ...
                <div class="container">
                    ...
                    <div class="nav-collapse">
                        <ul class="nav">
                        <li><a href="{$root}namespaces/RightNow.html">PHP Documentation</a></li>
                        <li><a href="{$root}symbols/RightNow.html">JS Documentation</a></li>
                        </ul>
                    </div>  
    ...
    <xsl:template match="/" mode="content-footer">
        <footer class="span12">
            ...
            Documentation is powered by <a href="http://www.phpdoc.org/">phpDocumentor <xsl:value-of select="$version"/></a>
        </footer>
    </xsl:template>
