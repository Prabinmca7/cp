...
class InlineImgTest extends CPTestCase {
    protected $attachmentAnswer = null;    

    function testGet() {
        $response = $this->makeRequest("/ci/InlineImg/get/foo/bar", array('justHeaders' => true));
    ...
    function testGuidGet() {
        ...
        $response = $this->makeRequest("/ci/InlineImg/guidGet", array('justHeaders' => true));
        $this->assertTrue(Text::stringContains($response, "400 Bad Request"));

        //test 400 response received when invalid guid value provided
        $response = $this->makeRequest("/ci/InlineImg/guidGet/0123456789", array('justHeaders' => true));
        $this->assertTrue(Text::stringContains($response, "400 Bad Request"));
        ...
        $response = $this->makeRequest("/ci/InlineImg/guidGet/12345678", array('justHeaders' => true));
