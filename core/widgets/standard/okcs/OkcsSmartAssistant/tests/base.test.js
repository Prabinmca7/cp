UnitTest.addSuite({
    ...
    var parentForm = Y.one('#form') !== null,
    ...
    smartAssistantTests.add(new Y.Test.Case({
        ...
        "Response Contains SA, but no SA data": function() {
            ...
            Y.Assert.isTrue(Y.one(dialog).ancestor('.yui3-panel').hasClass('rn_SmartAssistantDialogContainer'), "dialog doesn't have rn_SmartAssistantDialogContainer classname!");
        ...
        "Test the dialog's content when there's SA Data": function() {
            ...
            var accessKeyInstruction = Y.one(contents[0]).get("children")._nodes;
            Y.Assert.isNotNull(Y.one(accessKeyInstruction[0]));
            Y.Assert.isTrue(Y.one(accessKeyInstruction[0]).hasClass('rn_AccesskeyPrompt'));
