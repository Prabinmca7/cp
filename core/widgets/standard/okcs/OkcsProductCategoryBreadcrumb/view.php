        ...
        <ol aria-labelledby="rn_<?= $this->instanceID ?>_Label">
            ...
            <? $level = $this->data['levels'][$i];?>
                ...
                <li class="rn_BreadcrumbLevel rn_Level<?= $i ?> <?= $className ?>" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">
                    <? if($this->data['isAnswerDetail']): $className = ''; endif; ?>
                        <? if(!$className): ?>
                            <a href="<?= $this->data['attrs']['link_url'] ?>/<?= $this->data['paramKey'] ?>/<?= $level['id'] . \RightNow\Utils\Url::sessionParameter() ?>/<?= $this->data['paramKeyPc'] ?>/<?= $level['externalId'] ?>" itemprop="url">
                        <? else: ?>
                            <a itemprop="url">
                        <? endif; ?>
                        <span itemprop="title"><?= strip_tags($level['label']) ?></span>
                    </a>
                </li>
                </rn:block>
            <? endfor; ?>
