UnitTest.addSuite({
        ...
        setUp: function(){
            var testExtender = {
                initValues: function() {
                    ...
                    this.validateMaxLvl = function(lvl) {
                        var levelDiv = Y.one('.rn_Level' + lvl);
                        if (!levelDiv) return;

                        levelDiv.all('label').each(function(label) {
                            if (label.hasClass('rn_HasChildren')) {
                                Y.Assert.fail("The max level attribute has been violated!");
                            }
                        });
                    };
                }
    ...
    tests.add(new Y.Test.Case({
        ...
        testOpenAndClose: function() {
            ...
            this.validateMaxLvl(widget.data.attrs.max_lvl);
        },
    ...
    tests.add(new Y.Test.Case({
        ...
        testGetNoSelectionResponse: function() {
            this.initValues();
            this.validateResponse(widget._onValidateRequest("submit", [{data: {error_location: true}}]), 0);
        },
        ...
        testMultiSelectionResponse: function() {
            ...
            var test = function() {
                ...
                this.resume(function() {
                    ...
                    this.validateResponse(widget._onValidateRequest("submit", [{data: {error_location: true}}]), expectedValues.pop());
                    this.validateMaxLvl(widget.data.attrs.max_lvl);
                });
            };
            RightNow.Event.subscribe("evt_menuFilterGetResponse", test, this);
