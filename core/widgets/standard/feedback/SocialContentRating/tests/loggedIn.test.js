...
UnitTest.addSuite({
    ...
    suite.add(new Y.Test.Case({
        ...
        setUp: function () {
            ...
            this.ratingValue = Y.one(".rn_RatingValueNumerical");
        ...
        "Successful rating is submitted to the server": function () {
            ...
            Y.Assert.areSame(this.button.get('title'), widget.data.attrs.label_upvote_hint);
            ...
            // Call the callback.
            expectedArgs[2].successHandler.call(expectedArgs[2].scope, { ratingID: 'young', totalRatingLabel:{totalVotes: 1, label: 'Rating: 1 (1 user)'}});
            // Button is updated.
            Y.Assert.areSame(this.button.get('title'), widget.data.attrs.label_upvote_thanks);
            // And rating is reflected.
            Y.Assert.areSame('Rating: 1 (1 user)', this.getRatingValue());

        },

        "Rating is not submitted to the server if it's already selected": function () {
            widget._baseNode.addClass('rn_AlreadyRated');
            this.button.simulate('click');
