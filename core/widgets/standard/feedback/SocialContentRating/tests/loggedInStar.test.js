...
UnitTest.addSuite({
    ...
    suite.add(new Y.Test.Case({
        name: "UI Behavior For Star Voting",
        ...
        "Values of star voting buttons": function () {
            for (var $i=0; $i < 5; $i++) {
                Y.Assert.areSame((5-$i).toString(), this.button.item($i).get('value'));
            }
        },

        "Successful rating is submitted to the server": function () {
            this.button.item(4).simulate('click');

            Y.Assert.areSame(this.button.item(4).get('title'), widget.data.attrs.label_upvote_hint);
            ...
            // Call the callback.
            expectedArgs[2].successHandler.call(expectedArgs[2].scope, { ratingID: 'young', totalRatingLabel:{totalVotes: 1, label: 'Rating: 5/5 (1 user)'}});
            // Button is updated.
            Y.Assert.areSame(this.button.item(4).get('title'), widget.data.attrs.label_upvote_thanks);
            // And rating is reflected.
            Y.Assert.areSame('Rating: 5/5 (1 user)', this.getRatingValue());
