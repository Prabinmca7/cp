 ...
 * https://github.com/WebSergeevDima/php-simple-html-dom-parser/tree/master
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        ...
        foreach ($matches as $m) {
            ...
            if ($this->dom->lowercase) {$tag=strtolower($tag ? $tag : ''); $key=strtolower($key ? $key : '');}
    ...
    function convert_text($text) {
        ...
        $targetCharset = "";
        if ($this->dom) {
            $sourceCharset = strtoupper($this->dom->_charset ? $this->dom->_charset : '');
            $targetCharset = strtoupper($this->dom->_target_charset ? $this->dom->_target_charset : '');
        }
...
class simple_html_dom {
    ...
    protected function parse_charset()
    {
        ...
        if ((strtolower($charset ? $charset : '') == strtolower('ISO-8859-1')) || (strtolower($charset ? $charset : '') == strtolower('Latin1')) || (strtolower($charset ? $charset : '') == strtolower('Latin-1')))
        {
