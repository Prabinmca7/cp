...
class AbuseDetection
{
    ...
    public static function isAbuse($formToken=null) {
        if (self::$isAbuse === null || IS_UNITTEST) {
            ...
            {
                Api::phpoutlog("Abuse detection triggered in CP");
            }
        }
        return self::$isAbuse;
    }
    ...
    public static function isForceAbuseCookieSet() {
        return (isset($_COOKIE['isInAbuse']) && $_COOKIE['isInAbuse'] && $_COOKIE['isInAbuse'] === 'true');
    }
    ...
    {
        return strlen(isset($_POST['abuse_challenge_response']) && $_POST['abuse_challenge_response'] ? $_POST['abuse_challenge_response'] : '') ? array(
    ...
    private static function abuseData() {
        ...
        if (!isset($abuseData)) {
            ...
            $abuseData = array(
                isset($resource) && $resource ? $resource : "{$CI->router->fetch_class()}/{$CI->router->fetch_method()}",
     ...
     * @return void
     */
    public static function giveBonusHit() {
        $key = self::getBonusHitKey();
        if ($key) {
            $bonusKeyData = Api::memcache_value_deferred_get(MEMCACHE_TYPE_ADS, array($key));
            $bonusValue = Api::memcache_value_fetch(MEMCACHE_TYPE_ADS, $bonusKeyData);
            if(is_array($bonusValue) && isset($bonusValue[$key]) && $bonusValue[$key] == "1"){
                return;
            }
